...
#define REGISTER_COMPONENT(T, Name)                                                                                    \
    [[maybe_unused]] inline bool CONCAT(reg_component_, __COUNTER__) =                                                 \
        ly::internal::ComponentRegistrar<T, ly::internal::templated_string<sizeof(Name)>{Name}>::RegisterComponent();

#define SERIALIZE_BODY(T)                                                                                              \
    const char* Name = #T;                                                                                             \
    void SaveJson(cereal::JSONOutputArchive& archive) { save(archive); }                                               \
    static void LoadJson(cereal::JSONInputArchive& archive, T& component) { component.load(archive); }
