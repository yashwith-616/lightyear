...
public:
    ...
    template <>
    inline void SetUniform<glm::vec2>(const std::string& name, const glm::vec2& value) const {
        glUniform2fv(GetUniformLocation(name), 1, &value[0]);
    }

    // Specialization for glm::vec3
    template <>
    inline void SetUniform<glm::vec3>(const std::string& name, const glm::vec3& value) const {
        glUniform3fv(GetUniformLocation(name), 1, &value[0]);
    }

    // Specialization for glm::vec4
    template <>
    inline void SetUniform<glm::vec4>(const std::string& name, const glm::vec4& value) const {
        glUniform4fv(GetUniformLocation(name), 1, &value[0]);
    }

    // Specialization for glm::mat2
    template <>
    inline void SetUniform<glm::mat2>(const std::string& name, const glm::mat2& value) const {
        glUniformMatrix2fv(GetUniformLocation(name), 1, GL_FALSE, &value[0][0]);
    }

    // Specialization for glm::mat3
    template <>
    inline void SetUniform<glm::mat3>(const std::string& name, const glm::mat3& value) const {
        glUniformMatrix3fv(GetUniformLocation(name), 1, GL_FALSE, &value[0][0]);
    }

    // Specialization for glm::mat4
    template <>
    inline void SetUniform<glm::mat4>(const std::string& name, const glm::mat4& value) const {
        glUniformMatrix4fv(GetUniformLocation(name), 1, GL_FALSE, &value[0][0]);
    }

    inline void SetUniformBlock(const std::string& name,
